...
# Check if the user is on ChromeOS v117+ and is not in the VT-2 console (frecon)
if (( "${CHROMEOS_RELEASE_CHROME_MILESTONE}" > "116" )); then
...
if [ "$(grep -s "AuthenticAMD" /proc/cpuinfo)" ]; then
  echo_info "If the install fails, try running 'CREW_AMD_SEGFAULT=1 exec bash --init-file <(curl -Ls git.io/vddgY)'"
  if [ "${CREW_AMD_SEGFAULT}" == "1" ]; then
